...
### 2026-06-19 — The Regeneration Protocol
**Concept:** Inspired by Texas A&M's discovery that mammal regeneration isn't lost — just switched off (FGF2 + BMP2 sequential treatment regrows bone, cartilage, tendon, ligament, and nail in mice). You're reading a damaged lab notebook. Tissue sample sections are scarred over with fibrotic gibberish. Select (highlight) scarred text to apply FGF2; release the selection to trigger BMP2 differentiation. Text regenerates character-by-character under your cursor.
**Technique:** Selection API (`selectionchange` event, `window.getSelection()`, `Range.commonAncestorContainer`) as interactive game mechanic — the browser's native text selection becomes a therapeutic instrument. DOM tree walking from selection range to find containing tissue section. Character-level span animation for regeneration typing effect.
**File:** clients/web/src/pages/experiments/regeneration-protocol.astro
...
#### Selection API as Interactive Text Transformation Mechanic
`document.addEventListener("selectionchange", ...)` fires whenever the user's text selection changes — including when they highlight text by clicking and dragging, triple-clicking a paragraph, or using Shift+arrows. `window.getSelection()` returns the current `Selection` object; `.getRangeAt(0)` gives the `Range` with `.commonAncestorContainer` for identifying which element contains the selection. Walk up the DOM from the ancestor to find the interactive container. Key insight: the browser's native text selection becomes a surgical/therapeutic interaction — "selecting" scarred text triggers a transformation, and "deselecting" (mouseup elsewhere) completes it. Combine with `Range.getBoundingClientRect()` for spatial feedback. The `selectionchange` event fires on `document`, not on individual elements. Gotcha: `selectionchange` fires frequently during drag-select, so gate state transitions (e.g., only transition on first fire per target element). For character-by-character text regeneration, build spans dynamically and use staggered `animationDelay` for a typing effect.
